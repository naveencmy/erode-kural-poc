...
export default function ContentModule() {
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div>
        <h1 className="module-title tamil-text" style={{ fontSize: '1.05rem', fontWeight: 700 }}>{t('sidebar.content')}</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
        {TEMPLATES.map((tmpl) => (
          ...
          >
            <div style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--color-text-primary)' }} className="tamil-text">
              {t(tmpl.titleKey)}
            </div>
            <div style={{ fontSize: '0.88rem', color: 'var(--color-text-secondary)', marginTop: 4 }} className="tamil-text">
              {t(tmpl.descKey)}
            </div>
          </div>
        ))}
      ...
      <form onSubmit={handleGenerate} className="card" style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: '12px 16px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          <label className="tamil-text" style={{ fontSize: '0.95rem', fontWeight: 600, display: 'block', marginBottom: 4 }}>
            {t('content.subject')}:
          </label>
          <input
            type="text"
        ...
        <button type="submit" className="btn btn-primary" disabled={!subject.trim() || loading} style={{ alignSelf: 'flex-start', fontSize: '1rem' }}>
          <Sparkles size={16} />
          <span className="tamil-text" style={{ fontSize: '1rem' }}>{loading ? t('content.generating') : t('content.generate_btn')}</span>
        </button>

        {result && (
          <div style={{ padding: '12px 16px', background: 'var(--color-surface-hover)', borderRadius: 8, marginTop: 10 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--color-tn-success)' }}>
              <FileCheck size={18} />
              <span className="tamil-text" style={{ fontWeight: 700, fontSize: '0.95rem' }}>{t('content.success_title')}</span>
            </div>
            <p className="tamil-text" style={{ fontSize: '1rem', marginTop: 8 }}>
              {result.message}
            </p>
          </div>
        )}
